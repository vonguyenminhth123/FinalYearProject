...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        with dt[1]:
            ...
            """# Check for key press to stop webcam
            if cv2.waitKey(1) & 0xFF == ord('q'):
                break  # Break the loop to stop webcam"""
        # NMS
        with dt[2]:
                        ...
                        if names[c]:
                            if names[c] == 'awake':
                                ...
                                cv2.putText(im0, drowsy_text, (text_x_align, label_ymin - 7), cv2.FONT_HERSHEY_SIMPLEX,
                                            ...
                                            thickness,
                                            cv2.LINE_AA)
                                continuous_drowsy_time = 0
                                
                            elif names[c] == 'drowsy':
                                drowsy_arr.append([names[c]])
                                s_img = cv2.imread("icon/warning.png")
                                ...
                                cv2.putText(im0, drowsy_text, (text_x_align, label_ymin - 7), cv2.FONT_HERSHEY_SIMPLEX,
                                            ...
                                            cv2.LINE_AA)
                                while True:
                                    continuous_drowsy_time += 1
                                    if continuous_drowsy_time >= 5:
                                        drowsy_alert = True
                                        if drowsy_alert:
                                            pyttsx3.speak("Drowsiness Detected. Please stay alert!")
                                            continuous_drowsy_time = 0
                                        break
                                    break
                                ...
                                s_img = cv2.imread("icon/coffee.jpg")
                                s_img = cv2.resize(s_img, (72, 60), cv2.INTER_LINEAR)
                                im0[33:33 + s_img.shape[0], 10:10 + s_img.shape[1]] = s_img
